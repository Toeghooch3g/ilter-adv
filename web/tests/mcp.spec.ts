...
test.describe('MCP Servers page', () => {
  ...
  test('opens How To Connect panel with connection details', async ({ page }) => {
    ...
    await expect(page.getByText('Connect to Ilter Advanced MCP Hub')).toBeVisible({ timeout: 10000 })
    ...
    await expect(page.getByText('Connect to Ilter Advanced MCP Hub')).not.toBeVisible()
    await checkCrashIndicators(page, errors)
    expect(errors).toEqual([])
  })
