...
function DetailPanel({ entry, onClose }: { entry: McpAuditEntry; onClose: () => void }) {
  return (
    <DetailPanelShell title="Tool Call Details" onClose={onClose}>
      <div className="grid grid-cols-2 gap-4">
        ...
        <DetailField label="Tool" value={entry.tool} />
          ...
          <pre className="rounded-lg bg-surface-50 p-3 text-xs font-mono text-surface-700 overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap">
            {(() => {
              try {
                ...
                logger.error('McpAuditView: failed to parse params JSON for entry', entry.id)
                return entry.params
              }
            })()}
          </pre>
...
function McpAuditViewContent() {
  ...
  const queryParams = useMemo(() => {
    const p: { limit?: number; offset?: number; tool?: string; from?: string; to?: string; source: 'mcp' } = {
      limit,
      offset: (page - 1) * limit,
      source: 'mcp',
    }
  ...
  const {
    ...
    queryKey: [...qk.mcpAudit, page, toolFilter, timeRange, customFrom || '', customTo || ''],
  ...
  const filtered = entries.filter((e) => showDiscovery || e.method !== 'tools/list')

  const tools = useMemo(() => [...new Set(entries.map((e) => e.tool))].sort(), [entries])

  const stats = useMemo(() => {
    const totalCalls = filtered.length
    const errorCalls = filtered.filter((e) => !e.success).length
    const errorRate = totalCalls > 0 ? (errorCalls / totalCalls) * 100 : 0
    const avgDuration = totalCalls > 0 ? filtered.reduce((sum, e) => sum + e.duration_ms, 0) / totalCalls : 0
  ...
  const handleExportJson = () => {
    ...
    exportJson(exportData, `mcp-audit-${new Date().toISOString().slice(0, 10)}.json`)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          description="Per tool call"
          icon={<Clock className="w-5 h-5" />}
        />
        <StatCard
          title="Tools Used"
          value={isLoading ? '...' : tools.length}
          description="Unique tools in range"
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-40 rounded-lg border border-surface-200 bg-white pl-8 pr-3 py-1.5 text-xs text-surface-700 placeholder:text-surface-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
            />
          </div>
          {tools
            .filter((t) => !toolSearch || t.toLowerCase().includes(toolSearch.toLowerCase()))
            .map((t) => (
              <button
                key={t}
                onClick={() => {
                  setToolFilter(t === toolFilter ? '' : t)
                  setPage(0)
                }}
                className={cn(
      ...
      {isLoading ? (
        ...
        <LogsEmptyState
          ...
          description={toolFilter ? 'Try a different filter.' : 'No MCP tool calls have been recorded yet.'}
        />
      ) : (
        <Card>
            ...
            <div className="divide-y divide-surface-100">
              ...
              {filtered.map((entry) => (
                ...
                >
                  ...
                  <span className="col-span-2 font-mono text-sm text-surface-700 truncate">
                    <a
                      href="/mcp/servers"
                  ...
                  <span className="col-span-2 text-sm text-surface-700 flex items-center gap-1.5 min-w-0">
